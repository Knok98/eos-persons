...
export class FtpConnectService implements UploadService {
  private client: InstanceType<typeof SftpClient> | null = null;
  ...
  async connect() {
    ...
    if (!host || !username || !password) {
      throw new Error('Missing SFTP ENV (FTP_HOST / FTP_USER / FTP_PASSWORD)');
    }
  ...
  async uploadFile() {
    if (!this.client) throw new Error('SFTP client not initialized');
    ...
    try {
      ...
      await this.client.put(localCsvPath, remoteCsvPath);
